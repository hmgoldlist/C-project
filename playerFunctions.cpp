...
void getFarthestActionableLocation(const int maxActionDistance, const bool mustBeEmpty, int &locationX, int &locationY)
{
	...
	for (int i = 0; i < maxActionDistance; i++)
	{
		...
		switch(playerSymbol){
            case LOOKING_UP:
                if(locationY-1 >= 0 && playerIsLookingAt(locationX, locationY-1)){
                    locationY--;
                }
                break;
            case LOOKING_DOWN:
                if(locationY+1 <= MAP_WIDTH && playerIsLookingAt(locationX, locationY+1)){
                    locationY++;
                }
                break;
            case LOOKING_LEFT:
                if(locationX-1 >= 0 && playerIsLookingAt(locationX-1, locationY)){
                    locationX--;
                }
                break;
            case LOOKING_RIGHT:
                if(locationX+1 <= MAP_HEIGHT && playerIsLookingAt(locationX+1, locationY)){
                    locationX++;
                }
                break;
		}
...
bool playerIsLookingAt(const int x, const int y) // EREZ: correct
{
    ...
    if( playerX == lookX && playerX == x ){
        // Looking in Y direction
        if( lookY >= playerY && y >= lookY ){
            // Looking into positive Y direction
            for(i=playerY+1; i<y; i++){
                if(!canSeePast(x, i)){
                    return false;
                }
            }
            return true;
        }else if( lookY <= playerY && y <= lookY ){
            // Looking into negative Y direction
            for(i=playerY-1; i>y; i--){
                if(!canSeePast(x, i)){
                    return false;
                }
    ...
    }else if( playerY == lookY && playerY == y ){
        // Looking in X direction
        if( lookX >= playerX && x >= lookX ){
            // Looking into positive X direction
            for(i=playerX+1; i<x; i++){
                if(!canSeePast(i, y)){
                    return false;
                }
            }
            return true;
        }else if( lookX <= playerX && x <= lookX ){
            // Looking into negative X direction
            for(i=playerX-1; i>x; i--){
                if(!canSeePast(i, y)){
                    return false;
                }
